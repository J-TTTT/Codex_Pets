# Il-Capitano generation prompts

Generated with the built-in image generation tool, grounded in the approved Il-Capitano reference.

## base-pet.md

Create one clean full-body reference sprite for Codex pet Il-Capitano.

Pet identity: Il-Capitano exactreference: entirelyface-covering darkrigidhelmet withgoldgeometriccentralcrest/opening/sideornaments/chains; BLACK OPAQUE helmetinterior withNOskin,eyes,mouth,glowingeyes,screen,expressions. Blacklonghair, thickdarkgray animefurcollar, darkcape/longcoattails, blueblackinnerclothing, chestchains, armoredgloves andheavyboots. Differentiate darkmaterials withreference tonallevels. Hairandcape closebody withweight. Emptyhands, no weapons/shields/newprops. Exacthelmetshape, ornament/chains positions, fulltips/boots inside generousmargin. Neutralrelaxedstance; onlyframing/background/neutralpose adjustments. No effects,shadow,scene,text,watermark. Chroma-onlyoutsidecharacter; helmetinterior remainsOPAQUEblack,nevertransparenthole..
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `auto`: Infer the most appropriate pet-safe style from the user request and reference images, then keep that exact style consistent across every row. User style notes: Approvedreferenceonly: fine clean anime linework, soft celshading, subtlegradients andrestrained handpaintedmetal highlights. Same largehelmet, compactsturdybody, broadshoulders andshortlimbs. No photoreal,plastic/plush or3D..
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.


## 000.md

Repair one cardinal anchor for Codex pet `Il-Capitano`: `000` means looking up.
...
Repair one cardinal anchor for Codex pet `Il-Capitano`: `090` means looking right.
...
Repair one cardinal anchor for Codex pet `Il-Capitano`: `180` means looking down.
...
Repair one cardinal anchor for Codex pet `Il-Capitano`: `270` means looking left.
...
Create one horizontal four-cardinal anchor strip for Codex pet `Il-Capitano`.
...
Create Codex pet row `failed` for `Il-Capitano`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Il-Capitano exactreference: entirelyface-covering darkrigidhelmet withgoldgeometriccentralcrest/opening/sideornaments/chains; BLACK OPAQUE helmetinterior withNOskin,eyes,mouth,glowingeyes,screen,expressions. Blacklonghair, thickdarkgray animefurcollar, darkcape/longcoattails, blueblackinnerclothing, chestchains, armoredgloves andheavyboots. Differentiate darkmaterials withreference tonallevels. Hairandcape closebody withweight. Emptyhands, no weapons/shields/newprops. Exacthelmetshape, ornament/chains positions, fulltips/boots inside generousmargin. Neutralrelaxedstance; onlyframing/background/neutralpose adjustments. No effects,shadow,scene,text,watermark. Chroma-onlyoutsidecharacter; helmetinterior remainsOPAQUEblack,nevertransparenthole.. Preserve silhouette, face, palette, material, proportions, markings, and props.
...
Create Codex pet row `idle` for `Il-Capitano`: exactly 6 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `jumping` for `Il-Capitano`: exactly 5 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex v2 pet look row 10 for `Il-Capitano` as exactly 8 full-body frames in this order: 180, 202.5, 225, 247.5, 270, 292.5, 315, 337.5.
...
Create Codex v2 pet look row 9 for `Il-Capitano` as exactly 8 full-body frames in this order: 000, 022.5, 045, 067.5, 090, 112.5, 135, 157.5.
...
Create Codex pet row `review` for `Il-Capitano`: exactly 6 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `running-left` for `Il-Capitano`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `running-right` for `Il-Capitano`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `running` for `Il-Capitano`: exactly 6 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `waiting` for `Il-Capitano`: exactly 6 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `waving` for `Il-Capitano`: exactly 4 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create one horizontal animation strip for Codex pet `Il-Capitano`, state `failed`.
...
Identity: same pet in every frame: Il-Capitano exactreference: entirelyface-covering darkrigidhelmet withgoldgeometriccentralcrest/opening/sideornaments/chains; BLACK OPAQUE helmetinterior withNOskin,eyes,mouth,glowingeyes,screen,expressions. Blacklonghair, thickdarkgray animefurcollar, darkcape/longcoattails, blueblackinnerclothing, chestchains, armoredgloves andheavyboots. Differentiate darkmaterials withreference tonallevels. Hairandcape closebody withweight. Emptyhands, no weapons/shields/newprops. Exacthelmetshape, ornament/chains positions, fulltips/boots inside generousmargin. Neutralrelaxedstance; onlyframing/background/neutralpose adjustments. No effects,shadow,scene,text,watermark. Chroma-onlyoutsidecharacter; helmetinterior remainsOPAQUEblack,nevertransparenthole.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `auto`: Infer the most appropriate pet-safe style from the user request and reference images, then keep that exact style consistent across every row. User style notes: Approvedreferenceonly: fine clean anime linework, soft celshading, subtlegradients andrestrained handpaintedmetal highlights. Same largehelmet, compactsturdybody, broadshoulders andshortlimbs. No photoreal,plastic/plush or3D..
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.
Eightposes: helmetlowers, shouldersdrop slightly, briefpause, thenraishelmet/recoverstance. Nofacialfeatures/symbols/effects.


## idle.md

Create one horizontal animation strip for Codex pet `Il-Capitano`, state `idle`.
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.
Quiet chest/shoulderbreathing, tiny rigidhelmetadjustment, subtlehair/capesway, relaxedhands. NOBLINK/no facialfeatures.


## jumping.md

Create one horizontal animation strip for Codex pet `Il-Capitano`, state `jumping`.
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.
Fiveposes: slightkneebendanticipation, rise, smallairborneapex, descent, flexedkneessoftlanding. Actualverticalmotion, constantbodysize, no landingmarks/shadow.


## look-row-10.md

Create one horizontal look-direction strip for Codex pet `Il-Capitano`, atlas row 10.
...
EDIT TARGET GEOMETRY LOCK: The FIRST attached image is completed row 9. EDIT this whole strip into the eight LEFT-half poses above. Preserve the EXACT image canvas dimensions and source character full-body height, torso size, boot baseline and figure spacing of that target. Do not enlarge the row10 characters compared with row9. Redraw all eight helmet yaw/pitch poses together coherently, keeping body construction anchored; use approved DOWN and LEFT anchors for head direction. Never mirror or copy right-facing head poses. Row10 DOWN begins the existing down-right endpoint, ending up-left flows into existing UP. Maintain all features and fully opaque helmet interior. Green gutters remain wide. This is one unified 8-pose image edit, no individual patches.

CORRECTION OVERRIDES EARLIER FIRST-IMAGE DESCRIPTION: FIRST image now is rejected ROW10, the WHOLE-STRIP repair target. It already has correct fullbody height/baseline and seven usable direction families. SECOND image row9 is continuity reference only. Redraw the entire eight-frame row coherently, fixing the incorrect SECOND POSE: it must turn SLIGHTLY toward SCREEN LEFT while looking strongly DOWN. Its GOLD FRONT AXIS and POINTED BEAK must lie LEFT of the helmet mass center, unlike current image where they point RIGHT. DOWN cardinal first slot is centered, slot2 DOWN-LEFT follows toward slot3 LEFT-DOWN. NO right-facing helmets anywhere in row10. Keep all remaining poses in their existing left-half arc, preserve exact source canvas size/fullbody height/boots baseline and character identity; all8 must be generated together, no new individual-cell image. Use rigid helmet yaw and pitch, not distorted goldaxis or features. NO eyes or face. Maintain fully opaque dark interior. Whole-family consistency and progressive head orientation, stable body. Verify second slot clearly points LEFT before returning.

LATEST REPAIR: FIRST target now contains correct directions, including corrected DOWN-LEFT slot2. DO NOT change these directions. The defect is SPACING: second and third figures' cape/hair edges touch, forming one merged silhouette, plus a tiny detached speck. Redraw the WHOLE8POSE family with visible PURE GREEN gutters between every figure. Canvas remains2172x724. Preserve character HEIGHT about400-425pixels and same head/bodyratio/bootsbaseline; keep cape and hair closer inward to body so each full figure is at most240pixels wide within its271pixel slot. Minimum25px puregreen gap between neighboring figures. No accessories, hair, chain, cape tip or stray dot in gutters. Remove disconnected specks, no marks, no effects. This is physical cape/hair pose adjustment, do not squash/stretch body or shrink entirefigures. Keep original rigid helmet, goldornaments and opaque blackinterior. All8 coherent, head directions unchanged as LEFTarc. Finalcheck eight visually disjoint fullcharacters, with continuous green vertical separation corridors.


## look-row-9.md

Create one horizontal look-direction strip for Codex pet `Il-Capitano`, atlas row 9.
...
FINAL SEMANTIC REPAIR: FIRST attached image is the whole8pose row9 EDIT TARGET. Redraw this as ONE coherent eight-pose family, maintaining identical2172x724canvas and character heights~400-425px, fullbody proportions and planted boots. The current FIRST pose000 looks too frontal: make its helmet visibly tilt BACK and UP by roughly30degrees, chin/beak lifted toward TOP and away from furcollar; reveal a broad opaque dark underside of rigid helmet, foreshorten crown/top surface. Helmet frontal opening aims toward ceiling. NO eyes/skin/mouth added. Clear UP pitch must read without labels at192x208. Interpolate first four poses from this unmistakable UP to upper-right toward RIGHT; keep rightward goldaxis/beak on SCREEN-RIGHT side, natural rigidperspective, no warping. Last three gradually nod DOWN and right with crownmorevisible. Strengthen UP versus DOWN contrast using rigidhead pitch, not wholebody lean. Keep000fullbodyheight comparable to otherframes, no oversizedcrest, no enlargedneck. Maintain all8fullfigures separated bygreen corridors, keepcape/hair withinownslot withvisiblegaps, no specks or touches. Preserve exact goldcrest, sidechain attachment, fur/hair/costume and opaque faceless interior. No individualcellpatch: entire8pose family emitted as oneimage.


## review.md

Create one horizontal animation strip for Codex pet `Il-Capitano`, state `review`.
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.
Sixposes: lowhands, slightupperbodylean, helmetgentlyscansdifferentpositions thennods. Distinctfromhand-near-chinthinking; no newtools.


## running-left.md

Create one horizontal animation strip for Codex pet `Il-Capitano`, state `running-left`.
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.
Eight independently drawn shortsteadystrides facing IMAGE LEFT, alternatelegs, preserveoriginalhelmet/chains/outfit structure. Do NOTmirrorreference.


## running-right.md

Create one horizontal animation strip for Codex pet `Il-Capitano`, state `running-right`.
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.
Eight alternating shortsteadystrides facing IMAGE RIGHT, weightygentlebob, hair/cape/chains lag attached. Originalhelmetgeometry/costumeanatomy.


## running.md

Create one horizontal animation strip for Codex pet `Il-Capitano`, state `running`.
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.
SixTASKWORKposes: feetplanted, helmetslightlylowered, onegauntletnear lowerhelmetrim butneverinsidehelmet/fur, smallpauses/nods. NOTfootrunning.


## waiting.md

Create one horizontal animation strip for Codex pet `Il-Capitano`, state `waiting`.
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.
Sixposes: helmettilt, onehandopen infront, smalllean towarduser, patientawaitinggesture distinctfromidle.


## waving.md

Create one horizontal animation strip for Codex pet `Il-Capitano`, state `waving`.
...
Il-Capitano IDENTITY LOCK: exact approved helmet silhouette, goldcentralgeometriccrest, sideornaments, blacklonghair, thickdarkgray furcollar, blueblackchest/costume, goldchains, darkcape/tails, gauntlets andheavyboots. Same head/bodyproportions. Helmetfaceinterior is OPAQUE DARKmaterial: NOeyes,blink,mouth,skin,tears,expressionmarks,glowingeyes orscreens underANYcircumstance. Expressaction ONLY through body/helmetpose. Rigidhelmetshape preserved. Keepchainsattached, handsoutsidehelmet/fur, hair/capeheavynaturallag. Hands empty; no weapons/effects/shadow. Fineanimehanddrawncelstyle withdarktonalseparation. Alltips, fingers, hair, cape andboots complete.
SPACING: draw one fullbodyfigure centered in each invisibleequalwidthslot; entirefigurewidth atmost65percent ofslotwidth. Widepure #00FF00 gutters between EVERY neighboring hair/cape silhouette, withatleast25pxoutersafemargin. No touching/overlapping figures or visibleguide marks. Preservebodyratio ratherthanstretching tofillspace.
Fourtemporalposes: raise onegauntlet, smallgentlewave, slowsofthelmetnod; allfingersconnected, reservedknightlydemeanor.
